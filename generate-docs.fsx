...
let cleanXml (line: string) =
    line.Replace("///", "")
        ...
        .Replace("<param name=\"", "**Param ").Replace("\">", "**: ")
        .Replace("</param>", "")
        .Replace("<returns>", "**Returns**: ").Replace("</returns>", "")
        .Trim()
    ...
    if not (File.Exists(sourceFile)) then 
        ...
        for line in lines do
            ...
            if trimmed.StartsWith("///") then
                ...
                if not (String.IsNullOrWhiteSpace cleaned) then lastComments <- lastComments @ [cleaned]

            // 1. Better Module Detection (Captures name properly)
            elif (trimmed.StartsWith("module ") || trimmed.StartsWith("namespace ")) then
                let modName = trimmed.Replace("module ", "").Replace("namespace ", "").Replace("=", "").Trim()
                toc <- toc @ [sprintf "- [%s](#-module-%s)" modName (modName.ToLower().Replace(".", ""))]
            ...
            // 3. Cleaner Function List
            elif (line.StartsWith("let ") || line.StartsWith("    let ")) && not (trimmed.Contains(" = ")) then
                ...
                if parts.Length > nameIndex then
                    ...
                    let noise = ["sb"; "mutable"; "kf"; "nf"; "rf"; "sq"; "rsq"; "parts"; "rows"; "dirs"; "suites"]
            ...
            elif String.IsNullOrWhiteSpace trimmed then lastComments <- []

        let finalMarkdown = 
            ["# Floundroid Technical Reference"; "Generated on: " + DateTime.Now.ToString(); ""] 
            @ toc @ [""] @ content
