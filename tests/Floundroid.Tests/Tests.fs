...
module SearchTests =
    ...
    let ``MVV-LVA prefers Pawn takes Queen over Queen takes Pawn`` () =
        ...
        let getScore (m: Move) =
            ...
            | 1 -> 
                let v = Board.tryGetPiece b (Move.toSq m) |> Option.map (fun p -> Evaluation.matScore (Piece.kind p)) |> Option.defaultValue 100
                let a = Board.tryGetPiece b (Move.fromSq m) |> Option.map (fun p -> Evaluation.matScore (Piece.kind p)) |> Option.defaultValue 0
                10000 + (v * 10) - a
            | _ -> 0
